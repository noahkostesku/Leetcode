class Solution {
    public int minCostConnectPoints(int[][] points) {
        ...
        for (int i = 0; i < N; i++) {
            int x1 = points[i][0];
            int y1 = points[i][1];
            //want to connect current node to every other neighbour in the graph hence j starts at i + 1 not i
            for (int j = i + 1; j < N; j++) {
                int x2 = points[j][0];
                int y2 = points[j][1];
                int dist = Math.abs(x1 - x2) + Math.abs(y1 - y2);
                //two opposing lines for undirected graph need two connections
        ...
        //if the hashset and length of points is equal we've finished the algorithm
        while (visit.size() < N) {
            int[] curr = minH.poll();
            int cost = curr[0];
            int i = curr[1];
